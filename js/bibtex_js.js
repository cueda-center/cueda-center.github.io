...
function BibtexDisplay() {
    ...
    this.createStructure = function(structure, output, entries, level) {
        ...
        if (groupChild.length) {
            ...
            //This is for checking none grouped publications
...
function BibTeXSearcher() {
    ...
    this.checkEntry = function(entry, word) {
        var found = false;
        if (word[0] != "@") {
            entry.find("span:not(.noread)").each(function() {
            ...
            if (arrayStr.length < 2) {
                ...
                case "topic":
                    {
                        var count = 0;
                        entry.find("span:not(.noread)." + arrayStr[0]).each(function() {
                            count += 1;
    ...
    this.unhideAll = function() {
        $("div#bibtex_display").children().each(function() {
            $(this).show();
    ...
    this.searcher = function(input, needToRestart) {
        ...
        if (string.length) {
            ...
            else if (this.inputArray[this.inputArray.length - 1].length > splitInput[splitInput.length - 1].length) {
                needToRestart = true;
